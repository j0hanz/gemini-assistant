...
import type { StoreEntry, DocumentEntry } from '../../src/lib/store-registry.js';
...
describe('StoreRegistry', () => {
	...
	beforeEach(() => {
		...
		mockDocuments = new Map([
			[
				'stores/store-1',
				[
					{ name: 'files/doc-1', displayName: 'Doc 1', mimeType: 'text/plain', createTime: '2026-01-01T00:00:00Z' },
					{ name: 'files/doc-2', displayName: 'Doc 2', mimeType: 'text/plain', createTime: '2026-01-02T00:00:00Z' },
				],
			],
			[
				'stores/store-2',
				[
					{ name: 'files/doc-3', displayName: 'Doc 3', mimeType: 'application/pdf', createTime: '2026-01-03T00:00:00Z' },
				],
			],
		]);
	...
	it('listDocuments(storeName) returns correct documents for store', async () => {
		...
		const registry = createStoreRegistry({
			listStoresFn: async () => mockStores,
			listDocumentsFn: async (storeName: string) =>
				storeName === store1Name ? expectedDocs : [],
		});
	...
	it('DocumentEntry interface fields are preserved', async () => {
		...
		const registry = createStoreRegistry({
			listStoresFn: async () => mockStores,
			listDocumentsFn: async (storeName: string) =>
				storeName === store1Name ? store1Docs : [],
		});
