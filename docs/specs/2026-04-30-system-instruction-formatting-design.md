...
_Rationale:_ Tighten prose; formatting governed by base. Core constraint unchanged.
...
_Rationale:_ Adds prescribed skeleton; aligns citation format with Decision 8.
...
_Rationale:_ Adds lightweight skeleton; aligns citation format with Decision 8.
...
_Rationale:_ Adds skeleton; removes brittle exact-string instruction; aligns citation format.
...
_Rationale:_ Adds skeleton; tightens citation instruction.
...
_Rationale:_ Adds explicit no-prose rule; tightens code execution framing.
...
_Rationale:_ Replaces freeform sections with structured table; `Fix` column eliminates duplicate `## Fixes` section.
...
_Rationale:_ Adds structure to `## Differences`; applies table policy from Decision 3.
...
_Rationale:_ Structures each section with a purpose statement; applies citation format Decision 8; makes `## Notes` optional to reduce noise.
